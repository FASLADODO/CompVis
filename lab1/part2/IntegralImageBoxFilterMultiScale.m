function [ resultado ] = IntegralImageBoxFilterMultiScale( I0_d , sd_s , theta_corn , s , N )
% ---------------- 2.5.4 ----------------
for scale  = 1 : N
    curr_sd_s = s^(scale-1) * sd_s;
    ...
    result{scale} = [yS xS temp5];
    
    LaplacianOfGaussian = fspecial('log',n_s,curr_sd_s);
    LoG = imfilter(I0_d,LaplacianOfGaussian,'symmetric');
    LoG_norm{scale} = curr_sd_s^2 * abs(LoG); % normalized Laplacian = sigma_i^2 * Laplacian
end

final = [];

for scale = 1 : N
    temp6 = result{scale}(:, 1:2); % keep only coordinates of current scale examined 
	% keep only pixels that maximize Gaussian in a neighborhood of 2 
    if( scale == 1)
        for j = 1 : size( temp6(:,1) )
            Cond3 = LoG_norm{scale}(temp6(j,2),temp6(j,1)) > LoG_norm{scale+1}(temp6(j,2),temp6(j,1));
            if Cond3
                final = [ final ; result{scale}(j,:) ] ;
            end
        end
    elseif ( scale == N )
        for j = 1 : size( temp6(:,1) )
            Cond4 = LoG_norm{scale}(temp6(j,2),temp6(j,1)) > LoG_norm{scale-1}(temp6(j,2),temp6(j,1));
            if Cond4
                final = [ final ; result{scale}(j,:) ] ;
            end
        end
